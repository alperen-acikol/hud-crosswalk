...
USPS Crosswalk API and merging it with survey or administrative datasets.
...
**Key methods:**
- `huds_api()` — prompts user to enter HUD API token
...
### Retrieving Data from the HUD API

```python
from hud_crosswalk import DataLoad

dl = DataLoad()
dl.huds_api()                          # enter API token when prompted
params = dl.huds_url_modifier(2, query='All', year=2022, quarter=3)
huds_df = dl.api_call(params)
```

### Merging ZIP Codes with County FIPS Codes

```python
from hud_crosswalk import DataMerge

dm = DataMerge()
result = dm.run(survey_df, huds_df, df_zip='zip')
```

### Custom HUD Column Names
By default `DataMerge` assumes standard HUD column names. If your HUD data 
has different column names, override them at initialization:
